...
The formal ASN.1 module for this extension is defined in `spec/asn1/RiftExtension.asn1`.
...
Conformance test cases for transport security failure paths are defined in `spec/vectors/malformed-vectors.json`.
...
`media.playbackActionRequest` payload fields: `operationId`, `playbackId`, `sourceDeviceId`, `requestingDeviceId`, `action`, optional `positionMs`, optional `requestedAt` RFC 3339 timestamp. The envelope `operationId` MUST be present and equal the payload `operationId`. `operationId` is created by the requesting daemon, uniquely identifies this action request, and MUST be echoed unchanged by the corresponding result. `requestingDeviceId` MUST match the authenticated envelope identity. The v1 action vocabulary is closed: `play`, `pause`, `togglePlayPause`, `next`, `previous`, and `seek`. `positionMs` is REQUIRED only for `seek`.

`media.playbackActionResult` payload fields: `operationId`, `playbackId`, `sourceDeviceId`, `requestingDeviceId`, `action`, `success` boolean, optional `failureReason`, optional `message`. The envelope `operationId` MUST be present and equal the payload `operationId`. A media playback action result may complete only the operation whose `operationId` it echoes. Receivers MUST correlate results by `operationId`; the playback, source, requester, and action fields remain required validation fields and MUST NOT be used as the result correlation key. `requestingDeviceId` MUST match the original authenticated requester identity for the corresponding action request.
...
The formal ASN.1 module with OID assignment and IMPORTS is maintained in `spec/asn1/RiftExtension.asn1`.
...
Generated test vectors for these rejection classes are maintained in `spec/vectors/malformed-vectors.json`.
