...
The Test system now covers all 3 core cryptography/network subsystems with 27/27 tests passing:
...
2. **Fail-Closed Decoding (`decoder_test`):** Strictly verifies 8 advanced CVE-class attack vectors (missing OID, duplicate OID, critical flags, length manipulation, truncated DER).
3. **Stream Network Frame (`frame_codec_test`):** Verifies the 32 MiB lock mechanism and prevents double-parsing by directly returning Maps.
4. **Identity `identity_test`:** Verifies Atomic Write, Ed25519 PoP signature boundary enforcement (strict 32 bytes), memory zeroing (`dispose`), and the standard `rift-` Device ID string.

```bash
dart test
```
> **Case where 1 of 27 Tests Fails:**
