...
- `lib/src/network/`: 
  ...
  - `session_manager.dart`: Orchestrates the `session.hello` state machine. Evaluates PoP signatures using the *signer's own cert DER*, catches Zone exceptions natively, and accurately prunes offline peers to allow seamless reconnections.
...
- `test/`: Contains security test scenarios (Fail-Closed), including 35 passing tests across crypto, identity, pop, and frame handling.
...
The Test system now covers all core cryptography/network subsystems with 35/35 tests passing:
...
dart test
```
> **Case where 1 of 35 Tests Fails:**
