...
class WindowsMediaPlaybackPublisher {
  ...
  Future<void> start() async {
    ...
    _eventSub = WindowsMediaPlayback.events.listen(_handlePlaybackEvent);
    ...
    _connectionSub = _client.onConnectionChanged.listen((isConnected) {
      if (isConnected) {
        unawaited(WindowsMediaPlayback.startObservation());
      }
    });
    await WindowsMediaPlayback.startObservation();
  }
  ...
  Future<void> dispose() async {
    ...
    await WindowsMediaPlayback.stopObservation();
  }
  ...
  Future<void> _handlePlaybackEvent(Map<String, dynamic> event) async {
    ...
    try {
      ...
      debugPrint(
        '[Media Playback] Failed to publish Windows playback event: $error',
      );
    }
  }
  ...
  Future<void> _handleActionRequest(Map<String, dynamic> request) async {
    ...
    String? message;
    try {
      final result = await WindowsMediaPlayback.performAction(
        playbackId: playbackId,
        action: action,
