...
class IOSRemoteMediaPlaybackCoordinator {
  IOSRemoteMediaPlaybackCoordinator(this._client);
  ...
  Future<void> start() async {
    ...
    IOSMediaPlayback.setMethodCallHandler(handlePlatformMethodCall);
  ...
  Future<void> refresh() async {
    if (!IOSMediaPlayback.isSupported || !_client.isConnected) return;
    ...
    try {
      ...
      debugPrint('[iOS Media Playback] Failed to refresh state: $error');
    }
  }
  ...
  Future<void> _syncNativeState() async {
    ...
    if (playback == null) {
      await IOSMediaPlayback.clear();
      return;
    }

    await IOSMediaPlayback.show(playback: <String, Object?>{
