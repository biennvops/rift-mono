...
Rift is composed of three major components. First, the **Rift protocol specification**, a written, language-independent contract that defines wire formats, cryptographic operations, state machine transitions, and message schemas for every cross-device interaction. Second, two independent **daemon implementations** that conform to this specification: a C#/.NET 10 daemon (riftd) with Windows, macOS, and Linux hosts sharing one core implementation, and a Dart daemon hosted inside a Flutter foreground service on Android. Both daemons manage device identity, cryptographic trust, peer-to-peer encrypted communication, clipboard capability handling, and structured security event logging. Each daemon is also runnable as a standalone console process for development and continuous integration testing, with platform-specific service hosting treated as a thin lifecycle wrapper around the same daemon core. Third, the **Rift Flutter application**, a single Dart codebase that provides the user-facing experience for pairing, clipboard continuity, presence, and security event viewing. The capstone-core deliverable remains centered on Android plus Windows, while the active repository and validation scope now also include Linux and macOS desktop paths for the shared daemon and Flutter desktop verification. On desktop, the Flutter application is expected to remain resident when needed for clipboard monitoring and notifications.

The Flutter application communicates with whichever daemon it is paired with through a single JSON-RPC 2.0 IPC contract that is intentionally transport-agnostic: named pipes on Windows, Unix domain sockets on macOS and Linux, and `SendPort`/`ReceivePort` channels between Dart isolates on Android, with the same JSON-RPC framing applied on top in all cases. Because the same JSON-RPC contract is exercised regardless of host OS, the Flutter UI has exactly one client implementation that works against every supported daemon host, and the same conformance harness validates every daemon implementation. Protocol semantic equivalence between the two implementations is verified by automated conformance tests that run against both daemon implementations, plus cross-platform integration tests that exercise the Dart-on-Android daemon against the shared C# daemon stack across the active desktop hosts.
...
9. **Transport-Agnostic IPC for Platform Extension**:
   - The JSON-RPC 2.0 IPC contract between daemon and Flutter client is defined independently of any specific transport. In the current repository it is served over named pipes on Windows, Unix domain sockets on macOS and Linux, and over `SendPort`/`ReceivePort` channels between a UI Dart isolate and a dedicated background daemon Dart isolate on Android, with JSON-RPC framing applied on top so the contract is exercised consistently across transports. This design ensures that adding a new platform requires implementing only a new daemon host and a new transport binding, never changes to the protocol, the IPC contract, or the Flutter UI.

### 2. Proposed Solutions

The implementation of Rift involves two independent daemon backends (shared C#/.NET daemon hosts on Windows, macOS, and Linux; Dart on Android), a single cross-platform Flutter application, and a security-focused verification/documentation package. Below is a comprehensive breakdown of the proposed solutions:
...
- **Future Extension Boundary**:
  - Features such as relay transport, trust backup, account management, subscriptions, broader continuity flows, and additional platform targets beyond the current repository scope (especially iOS) may be documented as future extensions. The transport-agnostic IPC design is intended to make these additions straightforward without changing the protocol, the IPC contract, or the Flutter UI. iOS support specifically is expected to require Apple's Local Push Connectivity entitlement (`NEAppPushManager`), which is granted on a per-app review basis by Apple and is therefore noted as a future-work dependency outside the team's direct control. They are not required capstone-core deliverables and must not block demonstration of the software-engineering and security objectives.

- **Cross-Platform Flutter Application (Dart/Flutter)**:
  - Build a single Flutter application from one Dart codebase, with Material 3 and platform-adaptive widgets so the same UI scales between phone and desktop form factors. The capstone-core target pair is Android + Windows, while Linux/macOS desktop builds are now part of the active repository validation scope.
  - Implement a single Dart JSON-RPC 2.0 client (`JsonRpcRiftClient`) that consumes the daemon's IPC contract identically across supported targets. On Windows it connects to the riftd daemon over a named pipe; on macOS and Linux it connects over a Unix domain socket; on Android it connects to the dedicated background daemon Dart isolate over a `SendPort`/`ReceivePort` pair with JSON-RPC framing applied. The Flutter UI does not distinguish between these transports — all are JSON-RPC 2.0.
...
Requirement:
- Build the Flutter/Dart application with Material 3 and platform-adaptive layouts shared across the capstone-core Android/Windows targets, while keeping the same codebase extensible to the active Linux/macOS desktop validation paths in the repository.
...
- The JSON-RPC 2.0 IPC contract between daemon and Flutter client is intentionally transport-agnostic. In the current repository it is served over named pipes on Windows, Unix domain sockets on macOS and Linux, and over `SendPort`/`ReceivePort` isolate channels on Android (with JSON-RPC framing applied so the contract is exercised consistently). Adding a new platform requires implementing only a new daemon host and a new transport binding, with no changes to the protocol, the IPC contract, or the Flutter UI. iOS specifically is expected to require Apple's Local Push Connectivity entitlement (`NEAppPushManager`), which is granted on a per-app review basis by Apple and is therefore outside the team's direct control.
...
- The Flutter client is implemented as a single Dart codebase with a single `JsonRpcRiftClient`, distinguished only by which transport it binds to on each host. This consolidates client-side engineering effort onto one codebase while preserving the strength of the two-daemon protocol-conformance story on the daemon side.

- The team has identified the following as the minimum viable deliverable for capstone defense: written Rift protocol specification (including the exact ASN.1 structure of the custom Ed25519 X.509 extension), C# daemon on Windows as a Windows Service, Dart daemon on Android inside a Flutter foreground service with a dedicated background isolate, single Flutter UI on Android and Windows consuming both daemons through one JSON-RPC contract, the Flutter Windows application running tray-resident with minimize-on-close, mDNS-SD discovery and advertisement, mutual TLS encrypted communication with Ed25519 identity binding, pairing with fingerprint verification, clipboard offer/fetch between Android and Windows, basic presence, structured security event log, cross-implementation conformance tests, cross-platform interop tests, security tests including ASN.1 parser fuzz testing, and core security documentation. The repository may additionally contain macOS/Linux daemon hosts and desktop validation work; those strengthen the implementation story, but the capstone-core defense does not depend on every expanded platform path being feature-complete at the same level.
...
- iOS in-process daemon support and other additional platform targets beyond the current repository scope remain documented as future extensions. The transport-agnostic IPC design and the language-independent protocol specification are intended to make these additions straightforward without changing the capstone-core trust model.
